...
router.post('/', function(req, res, next) {
	...
	if(!personInfo.email || !personInfo.roll || !personInfo.password || !personInfo.passwordConf || !personInfo.course){
		res.send();
	} else {
	...
	User.findOne({unique_id:req.session.userId},function(err,data){
		...
		if(!data){
			...
			for (i in data.course){
				...
				Course.findOne({class_id:data.course[i], date:today },function(err,data){
					console.log(data);
					// if(!data){
			...
			return res.render('data.ejs', {"roll":data.roll,"email":data.email,"course":data.course});
		}
	});
});
...
router.post('/addCourse', function(req, res, next) {
	...
	if(!courseInfo.course || !courseInfo.date || !courseInfo.payload || !courseInfo.uuid || !courseInfo.dname){
		res.send();
	} else {
	
		Course.findOne({class_id:courseInfo.course, date:courseInfo.date},function(err,data){
			if(!data){
				var c;
				...
				res.send({"Success":"Attendance Registered for date: "+courseInfo.date+" and course: "+courseInfo.course});
			}else{
				res.send({"Success":"Attendance already registered for date: "+courseInfo.date+" and course: "+courseInfo.course});
			}
